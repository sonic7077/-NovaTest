...
export function renderReport(run, caseName) {
  const rows = run.steps.map((step) => `<tr><td>${escapeHtml(step.id)}</td><td>${escapeHtml(step.instruction || '-')}</td><td>${escapeHtml(step.status)}</td><td>${step.attempts}</td><td>${escapeHtml(step.error || '-')}<div class="evidence">${evidenceMarkup(run.id, step)}</div></td></tr>`).join('');
