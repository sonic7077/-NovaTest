...
describe('white-bag CMS smoke cases', () => {
  it('contains only readonly documented API actions', () => {
    ...
    expect(cases.flatMap((testCase) => testCase.steps.map((step) => step.request.action))).toEqual(expect.arrayContaining(['loginByPassword', 'config', 'list_post', 'list_post_comments', 'list_member_update_log']));
  });
});
